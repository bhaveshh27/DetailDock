...
{
    public class CreateProgramCommandHandler : IRequestHandler<CreateProgramCommand, IResponse>
    {
        ...
        public async Task<IResponse> Handle(CreateProgramCommand request, CancellationToken cancellationToken)
        {
            var programDto = _mapper.Map<ProgramDTO>(request);
            var response = await _programRepository.AddAsync(programDto);
            return response;
        }
    }
}
